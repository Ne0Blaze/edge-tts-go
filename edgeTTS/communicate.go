			...
			case websocket.TextMessage:
				...
				if path == "turn.start" {
					...
					if err := json.Unmarshal(data, meta); err != nil {
						log.Printf("We received a text message, but unmarshal failed.")
					}
					for _, v := range meta.Metadata {
						if v.Type == ChunkTypeWordBoundary {
							...
							log.Printf("Unknown metadata type: %s", v.Type)
						}
					}
				} else if path != "response" {
					log.Printf("The response from the service is not recognized.\n%s", data)
				}
			case websocket.BinaryMessage:
				if !downloadAudio {
					log.Printf("We received a binary message, but we are not expecting one.")
				}
				if len(data) < 2 {
					log.Printf("We received a binary message, but it is missing the header length.")
				}
				headerLength := int(binary.BigEndian.Uint16(data[:2]))
				if len(data) < headerLength+2 {
					log.Printf("We received a binary message, but it is missing the audio data.")
				}
				text.chunk <- communicateChunk{
					Type: ChunkTypeAudio,
		...
		for {
			...
			if ok {
				if v.Type == ChunkTypeAudio {
					...
					t.speechMarks = append(t.speechMarks, SpeechMark{
						Time:  v.Offset / 10000,
						Value: v.Text,
					})
